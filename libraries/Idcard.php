 ...
 * 身份证工具类
 * 
 * 支持香港、台湾地区身份证验证；支持通过身份证号获取年龄、性别、省份
 * @author ChenHao <cnxbl@qq.com> 2014-09-22
 * @copyright Copyright (c) 2014 chedaye.com
 * @license http://www.chedaye.com
 * @version $Id$
 */

namespace mycone\users\libraries;
...
class Idcard {
	...
	*/
	...
	public static function validateCard($idCard) {
		if (self::validateIdCard18 ( $idCard )) {
			return true;
		}
		if (self::validateIdCard15 ( $idCard )) {
			return true;
		}
		$cardval = self::validateIdCard10 ( $idCard );
		if ($cardval != null) {
			if ($cardval [2] == "true") {
				return true;
			}
	 ...
	 * @return 验证码是否符合
	 */
	public static function validateHKCard($card) {
